...
int main(int argc, char **argv) {
    ...
    // Nombre para la cola del servidor. Al concatenar el login sera unica en un sistema compartido.
    sprintf(serverQueue2, "%s-%s", SERVER_QUEUE2, getenv("USER"));
    printf("[Cliente]: El nombre de la cola del servidor es: %s\n", serverQueue2);
    ...
    mq_server2 = mq_open(serverQueue2, O_RDONLY);
    ...
    if (mq_server2 == (mqd_t)-1) { // Comprobamos que se haya abierto correctamente
        perror("Error al abrir la cola del servidor");
        funcionLog("Error al abrir la cola del servidor");
        exit(-1);
    }
    ...
    do {
        ...
        */
        ...
        bytes_read2=mq_receive(mq_server2, readbuffer2, MAX_SIZE, NULL);
